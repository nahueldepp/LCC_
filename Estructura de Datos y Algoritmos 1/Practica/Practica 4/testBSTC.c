...
int main() {
    ...
    printf("bstb: %d\n",bstree_buscar(arbol, "farmacia", comparar_cadena) );
    // Buscar elementos
    ...
    BSTree nodo= bstree_minimo_derecho(arbol);
    //..............printf("%s\n",(char*) nodo->data);
    arbol= bstree_eliminar(arbol,"farmacia",(FuncionComparadora) comparar_cadena, (FuncionDestructora) destruir_cadena);
    bstree_recorrer(arbol, BTREE_RECORRIDO_IN, imprimir_cadena, NULL);
    puts(""); 
    
    BSTree kesimo= bstree_k_esimo_menor( arbol, 6);
    printf("kesimo %s\n", (char*)kesimo->data);

    // arbol valido
    int i=bstree_ABB_valido(arbol, (FuncionComparadora) comparar_cadena);
    printf("valido: %d\n", i);
    free(arbol->der->izq->data);
    arbol->der->izq->data= copiar_cadena("edia");
    int j=bstree_ABB_valido(arbol, (FuncionComparadora) comparar_cadena);
    printf("valido2: %d\n", j);
    printf("arbol cambio: %s\n", (char*)arbol->der->izq->data);
     bstree_recorrer(arbol, BTREE_RECORRIDO_IN, imprimir_cadena, NULL);
    puts(""); 
