...
/**
 * busca el elemento dado en la lista, retorna el nodo si lo encuentra, NULL en otro caso
 */

GNode* glist_buscar(GList list, FuncionComparadora comp ,void* dato) {
  GNode* encontrado=NULL;
  for (GNode *node = list; node != NULL && !encontrado; node = node->next){
    if(comp(node->data, dato)==0){
      encontrado=node;
    }
  }
...
/**
 * Elimina el nodo que contiene el dato dado
 */
GList glist_eliminar_nodo(GList lista,void* dato, FuncionComparadora comp, FuncionDestructora destr){
  if(lista == NULL)
    return lista;
  if(comp(lista->data, dato)==0){
    GList temp=lista->next;
    destr(lista->data);
    free(lista);
    return temp;
  }
  lista->next= glist_eliminar_nodo(lista->next, dato, comp, destr);
  return lista;
}
